    ...
    return (
        <div className='cardContainer'>
            {Object.keys(eventsListData).map(key => {
                ...
                switch (filter) {
                    case "ALL": if (accessPost) comp = shortComp; break;
                    case "DATE": if (date === handleDate(id) && accessPost) comp = shortComp; break;
                    case "PAST": if (date < handleDate(getDateToday()) && accessPost) comp = shortComp; break;
                    case "FUTURE": if (date > handleDate(getDateToday()) && accessPost) comp = shortComp; break;
